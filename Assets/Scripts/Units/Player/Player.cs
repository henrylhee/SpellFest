...
public class Player : Unit
{
    ...
    protected override void Initialize()
    {
        ...
        if (SaveSystem.Instance.GetCurrentCheckPointIndex() > 0)
        {
            transform.position = SaveSystem.Instance.GetPlayerStartPosition();
        }
        if (SaveSystem.Instance.IsFireSpellUpgraded())
        {
            fireProjectile.GetComponent<ElementProjectile>().Upgrade();
        }
        if (SaveSystem.Instance.IsIceSpellUpgraded())
        {
            iceProjectile.GetComponent<ElementProjectile>().Upgrade();
        }
    }
    ...
    {
        fireProjectile.GetComponent<ElementProjectile>().Upgrade();
        SaveSystem.Instance.UpgradeFireSpell();
    }

    public void UpgradeIceSpell()
    {
        iceProjectile.GetComponent<ElementProjectile>().Upgrade();
        SaveSystem.Instance.UpgradeIceSpell();
    }
